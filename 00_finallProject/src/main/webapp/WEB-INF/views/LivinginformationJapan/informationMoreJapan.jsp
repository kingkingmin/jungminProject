	...
	function successcall(data) {
		...
		$.each(data, function(i) {
			...
			if (hh === $("#i2").val()) {
				...
				$('#mainTbl2').append(" ブロックされました。コメントは不可能です。 ");
...
<body >
			...
			<table id="inforWritetable2" >
				...
				<tr>
					<td colspan="2" ><b>  ${inforboard.f_categoryJ} / [${inforboard.f_sortJ}] </b></td>
				...
				<tr>
					<td colspan="3" ><b>作成日 : </b><fmt:formatDate value="${inforboard.f_date}" type="both" dateStyle="short" timeStyle="short"/></td>
					<td colspan="3" ><b>作成者 : </b> ${inforboard.f_writter}</td>
				...
				<td colspan="6" >${inforboard.f_story}  </td>
			...
			<tr>
			<td colspan="2">
				<button type="button" onclick="gobackinfor()" class="button4">前のページへ戻る</button>
			</td>
			<td colspan="2">
			 ...
			 <c:if test="${sessionScope.loginMember.c_id == inforboard.f_writter }">
				...
				<button type="button" onclick="checkdelect(${inforboard.f_no })" class="button">削除</button>
			...
			<td colspan="2">
			 ...
			 <c:if test="${sessionScope.loginMember.c_id == inforboard.f_writter }">
				...
				<button class=buttonInforUpdate>修正</button>
				</form>
					...
					<td colspan="6" id="smReplyArea">
						...
						<c:choose>
							<c:when test="${repl.sir_c_writer == sessionScope.loginMember.c_id }">
								<span ondblclick="snsReplyDelete('${repl.sir_no}');" class="InforReplyIDMe" style=" cursor:pointer;" >${repl.sir_c_writer } さん  </span>	
								...
								<input name="sir_no" value="${repl.sir_no}" type="hidden">
								 <input name="f_no" value="${inforboard.f_no }" type="hidden"> 
									...
									<button class="button4">削除</button>
								</form>
			...
			<c:if test="${sessionScope.loginMember.c_id != null}">
					...
					<form action="sns.write.reply"
						...
						<input name="sir_c_no" value="${inforboard.f_no }" type="hidden"> 
 						<input name="f_no" value="${inforboard.f_no }" type="hidden"> 
						<textarea id="snsCom" name="sir_c_comment" style="width: 500px; height: 90px;" placeholder="コメントを入力してください。" onfocus="this.placeholder=''" onblur="this.placeholder='コメントを入力してください。'"></textarea>
						&nbsp;<button class="button">コメント書く</button>
